...
function Dashboard() {
  ...
  const systemOverview = {
    ...
    irrigationActive: 2,
    waterUsedToday: 1247,
    waterSavedThisMonth: 8934,
    alertsCount: 3,
    batteryAverage: 76,
    lastSystemCheck: '2 minutes ago'
  };

  const criticalAlerts = [
    { id: 1, device: 'Field C - Zone 1', type: 'offline', message: 'Device offline for 2 hours', severity: 'high', time: '2 hours ago' },
    { id: 2, device: 'Field A - Zone 1', type: 'battery', message: 'Low battery (23%)', severity: 'medium', time: '1 hour ago' },
    { id: 3, device: 'Field D - Zone 1', type: 'maintenance', message: 'Scheduled maintenance due', severity: 'low', time: '1 day ago' }
  ];

  const activeIrrigation = [
    { id: 1, device: 'Field A - Zone 1', startTime: '14:30', duration: '45 min', progress: 75, waterUsed: 234 },
    { id: 2, device: 'Field B - Zone 2', startTime: '15:00', duration: '30 min', progress: 40, waterUsed: 156 }
  ];

  const recentReadings = [
    { id: 1, device: 'Field A - Zone 1', moisture: 45, temp: 22, ph: 6.8, status: 'optimal', time: '2 min ago', irrigating: true },
    { id: 2, device: 'Field B - Zone 2', moisture: 42, temp: 23, ph: 6.7, status: 'optimal', time: '3 min ago', irrigating: true },
    { id: 3, device: 'Field A - Zone 3', moisture: 78, temp: 24, ph: 6.9, status: 'high', time: '3 min ago', irrigating: false },
    { id: 4, device: 'Field C - Zone 1', moisture: 23, temp: 25, ph: 7.0, status: 'critical', time: '2 hours ago', irrigating: false },
    { id: 5, device: 'Field D - Zone 1', moisture: 0, temp: 0, ph: 0, status: 'offline', time: '1 day ago', irrigating: false }
  ];

  const weatherData = {
    current: { temp: 24, humidity: 65, windSpeed: 12, condition: 'Partly Cloudy' },
    forecast: [
      { day: 'Today', high: 26, low: 18, rain: 10, icon: '⛅' },
      { day: 'Tomorrow', high: 28, low: 20, rain: 0, icon: '☀️' },
      { day: 'Wed', high: 25, low: 17, rain: 60, icon: '🌧️' },
      { day: 'Thu', high: 23, low: 15, rain: 80, icon: '⛈️' }
    ]
  };

  const farmMetrics = {
    totalArea: '125 acres',
    cropTypes: ['Corn (60 acres)', 'Soybeans (45 acres)', 'Wheat (20 acres)'],
    soilHealth: 85,
    waterEfficiency: 92,
    yieldPrediction: '+12%'
  };
          ...
          <div className="bg-red-50 border border-red-200 rounded-xl p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="font-medium text-red-900">{criticalAlerts.length} Active Alert{criticalAlerts.length > 1 ? 's' : ''}</p>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200">
              ...
              <div className="p-6">
                <div className="text-center mb-4">
                  <p className="text-3xl font-bold text-gray-900">{weatherData.current.temp}°C</p>
                  <p className="text-gray-600">{weatherData.current.condition}</p>
                  <p className="text-sm text-gray-500">Humidity: {weatherData.current.humidity}% • Wind: {weatherData.current.windSpeed} km/h</p>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200">
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <p className="font-semibold text-gray-900">{farmMetrics.totalArea}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-2">Crop Distribution</p>
                  {farmMetrics.cropTypes.map((crop, index) => (
                    <p key={index} className="text-sm text-gray-900">{crop}</p>
                  ))}
                </div>
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-green-600">{farmMetrics.soilHealth}%</p>
                    <p className="text-xs text-gray-600">Soil Health</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-blue-600">{farmMetrics.waterEfficiency}%</p>
                    <p className="text-xs text-gray-600">Water Efficiency</p>
                  </div>
                </div>
                <div className="text-center pt-2">
                  <p className="text-lg font-bold text-green-600">{farmMetrics.yieldPrediction}</p>
                  <p className="text-xs text-gray-600">Predicted Yield Increase</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 sm:p-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-3 gap-2 sm:gap-0">
                <span className="text-sm text-gray-500">{criticalAlerts.length} active alert{criticalAlerts.length !== 1 ? 's' : ''}</span>
            ...
            {criticalAlerts.length === 0 ? (
              ...
              <div className="space-y-3 sm:space-y-4">
                {criticalAlerts.map((alert) => (
                  <div key={alert.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 hover:bg-gray-50 rounded-lg transition-colors border border-gray-200 gap-3 sm:gap-4">
                    {/* Alert Info Section */}
                    <div className="flex items-start sm:items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
                      <div className="flex items-center space-x-2 sm:space-x-3 flex-shrink-0">
                        ...
                        <div className={`p-1.5 sm:p-2 rounded-lg ${
                          alert.type === 'offline' ? 'bg-red-100' :
                          alert.type === 'battery' ? 'bg-yellow-100' : 'bg-blue-100'
                        }`}>
                          {alert.type === 'offline' && (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-3 mb-1">
                          <p className="font-semibold text-gray-900 text-sm sm:text-base truncate">{alert.device}</p>
                          <span className={`inline-flex items-center px-2 py-0.5 sm:px-2.5 sm:py-0.5 rounded-full text-xs font-medium border mt-1 sm:mt-0 self-start ${getSeverityColor(alert.severity)}`}>
                            {alert.severity.toUpperCase()}
                          </span>
                        </div>
                        <p className="text-sm text-gray-700 mb-1 break-words">{alert.message}</p>
